 ...
 * @author dosse
 */
public class ClientConnectionMic extends Thread {

    private ServerBroadcastMic serv; //instance of server, needed to put messages in the server's broadcast queue
    ...
    private ArrayList<MessageMic> toSend = new ArrayList<MessageMic>(); //queue of messages to be sent to the client
    ...
    public ClientConnectionMic(ServerBroadcastMic serv, Socket s) {
        this.serv = serv;
        this.s = s;
    ...
    public void addToQueue(MessageMic m) { //add a message to send to the client
        try {
            toSend.add(m);
        } catch (Throwable t) {
    ...
    public void run() {
        try {
            ...
            try {
                s.close();
                SoundPacket.Log.add("ERROR " + getInetAddress() + ":" + getPort() + " " + ex);
        ...
        for (;;) {
            try {
                if (s.getInputStream().available() > 0) { //we got something from the client
                    MessageMic toBroadcast = (MessageMic) in.readObject(); //read data from client
                ...
                try {
                    if (!toSend.isEmpty()) {
                        MessageMic toClient = toSend.get(0); //we got something to send to the client
                        if (!(toClient.getData() instanceof SoundPacket) || toClient.getTimestamp() + toClient.getTtl() < System.nanoTime() / 1000000L) { //is the message too old or of an unknown type?
                            SoundPacket.Log.add("dropping packet from " + toClient.getChId() + " to " + chId);
                            continue;
                        }

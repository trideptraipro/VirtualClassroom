            ...
            if (mic.available() >= SoundPacket.defaultDataLenght) { //we got enough data to send
                ...
                try {
                    ...
                    if (tot == 0) {//send empty packet
                        m = new MessageMic(-1, -1, new SoundPacket(null));
                    } else { //send data
                        ...
                        baos.flush();
                        baos.close();
                        m = new MessageMic(-1, -1, new SoundPacket(baos.toByteArray()));  //create message for server, will generate chId and timestamp from this computer's IP and this socket's port
                    }
